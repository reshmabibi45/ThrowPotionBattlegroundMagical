...
struct BallThrowGameView: View {
    ...
    @State private var isDraggingPaddle = false
            ...
            ZStack {
                ...
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 20) {
                        ...
                        GameAreaView(gameManager: gameManager, isDraggingPaddle: $isDraggingPaddle)
                            .frame(height: calculateGameAreaHeight(for: geometry.size))
                            .padding(.horizontal, 20)
                            .background(Color.black.opacity(0.2))
                            .cornerRadius(16)
                            .padding(.horizontal, 20)
                            .disabled(gameManager.gameState.ballVelocity != .zero)
                        ...
                        .padding(.horizontal, 40)
                        .padding(.bottom, 50) // Increased bottom padding for better visibility
                ...
                // Disable scrolling when dragging paddle
                .simultaneousGesture(
                    DragGesture()
                        .onChanged { _ in
                            if isDraggingPaddle {
                                // Prevent scroll view from scrolling when dragging paddle
                            }
                        }
                )
...
struct GameAreaView: View {
    @ObservedObject var gameManager: BallThrowGameManager
    @Binding var isDraggingPaddle: Bool
    @State private var dragOffset: CGSize = .zero
    @State private var lastPaddlePosition: CGFloat = 0
            ...
            ZStack {
                ...
                RoundedRectangle(cornerRadius: 10)
                    ...
                    .position(
                        ...
                        y: geometry.size.height - calculatePaddleHeight(for: geometry.size) / 2 // Fixed at bottom
                    )
                    ...
                    .gesture(
                        DragGesture(minimumDistance: 0)
                            .onChanged { value in
                                ...
                                // Calculate paddle position with bounds
                                let paddleHalfWidth = calculatePaddleWidth(for: geometry.size) / 2
                                let newPosition = min(
                                    max(value.location.x, paddleHalfWidth),
                                    geometry.size.width - paddleHalfWidth
                                )
                                
                                // Smooth movement by limiting the distance moved per frame
                                let maxMovement: CGFloat = 50 // Adjust this value for smoother movement
                                let positionDifference = abs(newPosition - lastPaddlePosition)
                                
                                if positionDifference <= maxMovement {
                                    gameManager.movePaddle(to: newPosition)
                                } else {
                                    let direction: CGFloat = newPosition > lastPaddlePosition ? 1 : -1
                                    gameManager.movePaddle(to: lastPaddlePosition + (direction * maxMovement))
                                }
                                
                                lastPaddlePosition = gameManager.gameState.paddlePosition
                            }
                            .onEnded { _ in
