    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 20) {
                ...
                // Game Area with proper geometry
                GameAreaView(gameManager: gameManager)
                    .frame(height: 500) // Increased height for better gameplay
                    .padding(.horizontal, 20)
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack {
                ...
                // Targets - All 6 items properly positioned
                ForEach(gameManager.throwableItems) { item in
                    if !item.isDestroyed {
                        VStack(spacing: 2) {
                            Text(item.element.rawValue)
                                .font(.system(size: 24)) // Smaller font for smaller items
                ...
                RoundedRectangle(cornerRadius: 10)
                    ...
                    .position(
                        ...
                        y: geometry.size.height - 20 // Fixed at bottom
                    )
                    ...
                    .gesture(
                        DragGesture()
                            .onChanged { value in
                                ...
                                let paddleRect = CGRect(
                                    x: gameManager.gameState.paddlePosition - 50, // half width
                                    y: geometry.size.height - 30, // slightly above bottom
                                    width: 100,
                                    height: 30
                                )
            ...
            .gesture(
                DragGesture()
                    .onChanged { value in
                        ...
                        let paddleYRange = (geometry.size.height - 50)...geometry.size.height
